# Differential expression analysis for Cluster 7 vs 6
...
deg_7vs6 <- split(df, df$diffexpressed)
...
# Faecal untargeted metabolomics 
FC_fecal_7vs6 <- foldChngCalc(fmetab, 16, 13) 
p_fecal_7v6 <- NULL
for (i in 1:dim(fmetab)[1]){
  p_fecal_7v6[i]<-p.valcalc(as.numeric(fmetab[i,1:16]),as.numeric(fmetab[i,17:29]))
}

p_fecal_7v6 <- p.adjust(p_fecal_7v6, "BH", n = length(p_fecal_7v6) + dim(scfa)[1] + dim(bile)[1])

res_fmetab_7vs6 <- cbind(FC_fecal_7vs6, p_fecal_7v6) %>%
  as.data.frame() %>%
  dplyr::rename(log2FoldChange = FC_fecal_7vs6, pvalue = p_fecal_7v6)

res_fmetab_7vs6$log2FoldChange <- abs_log(res_fmetab_7vs6$log2FoldChange)
output_f_untargeted <- EnhancedVolcano(res_fmetab_7vs6, pCutoff = 0.05,
                                       lab = rownames(res_fmetab_7vs6),
                                       x = 'log2FoldChange',
                                       y = 'pvalue')
# Bile acid 
FC_bile_7vs6 <- foldChngCalc(bile, 16, 19) 

p_bile_7vs6 <- NULL
for (i in 1:dim(bile)[1]){
  p_bile_7vs6[i]<-p.valcalc(as.numeric(bile[i,1:16]),as.numeric(bile[i,17:35]))
}

p_bile_7vs6 <- p.adjust(p_bile_7vs6, "BH", n = length(p_bile_7vs6) + dim(scfa)[1] + dim(bile)[1])

res_bile_7vs6 <- cbind(FC_bile_7vs6, p_bile_7vs6) %>%
  as.data.frame() %>%
  dplyr::rename(log2FoldChange = FC_bile_7vs6, pvalue = p_bile_7vs6)

res_bile_7vs6$log2FoldChange <- abs_log(res_bile_7vs6$log2FoldChange)

output_f_bile <- EnhancedVolcano(res_bile_7vs6, pCutoff = 0.05,
                                 lab = rownames(res_bile_7vs6),
                                 x = 'log2FoldChange',
                                 y = 'pvalue')

# SCFA
FC_scfa_7vs6 <- foldChngCalc(scfa, 21, 20) 

p_scfa_7vs6 <- NULL
for (i in 1:dim(scfa)[1]){
  p_scfa_7vs6[i]<-p.valcalc(as.numeric(scfa[i,1:21]),as.numeric(scfa[i,22:41]))
}

p_scfa_7vs6 <- p.adjust(p_scfa_7vs6, "BH", n = length(p_bile_7vs6) + dim(scfa)[1] + dim(bile)[1])

res_scfa_7vs6 <- cbind(FC_scfa_7vs6, p_scfa_7vs6) %>%
  as.data.frame() %>%
  dplyr::rename(log2FoldChange = FC_scfa_7vs6, pvalue = p_scfa_7vs6)

res_scfa_7vs6$log2FoldChange <- abs_log(res_scfa_7vs6$log2FoldChange)

output_f_scfa <- EnhancedVolcano(res_scfa_7vs6, pCutoff = 0.05,
                                 lab = rownames(res_scfa_7vs6),
                                 x = 'log2FoldChange',
                                 y = 'pvalue')

# Insufficient differentially abundant fecal metabolites to export ----
...
# Untargeted
FC_pmetab_7vs6 <- foldChngCalc(pmetab_lab, 15, 10) 

p_pmetab_7vs6 <- NULL
for (i in 1:dim(pmetab_lab)[1]){
  p_pmetab_7vs6[i]<-p.valcalc(as.numeric(pmetab_lab[i,1:15]),as.numeric(pmetab_lab[i,16:25]))
}

p_pmetab_7vs6 <- p.adjust(p_pmetab_7vs6, "BH", n = length(p_pmetab_7vs6) + dim(amino_lab)[1])
res_pmetab_7vs6 <- cbind(FC_pmetab_7vs6, p_pmetab_7vs6) %>%
  as.data.frame() %>%
  dplyr::rename(log2FoldChange = FC_pmetab_7vs6, pvalue = p_pmetab_7vs6)

res_pmetab_7vs6$log2FoldChange <- abs_log(res_pmetab_7vs6$log2FoldChange)

output_p_untargeted <- EnhancedVolcano(res_pmetab_7vs6, pCutoff = 0.05,
                                       lab = rownames(res_pmetab_7vs6),
                                       x = 'log2FoldChange',
                                       y = 'pvalue')
# Amino
FC_pmetab_7vs6_amino <- foldChngCalc(amino_lab, 11, 14) 

p_pmetab_7vs6_amino <- NULL
for (i in 1:dim(amino_lab)[1]){
  p_pmetab_7vs6_amino[i]<-p.valcalc(as.numeric(amino_lab[i,1:11]),as.numeric(amino_lab[i,12:25]))
}

p_pmetab_7vs6_amino <- p.adjust(p_pmetab_7vs6_amino, "BH", n = length(p_pmetab_7vs6_amino) + length(res_pmetab_7vs6$pvalue))

res_amino_7vs6 <- cbind(FC_pmetab_7vs6_amino, p_pmetab_7vs6_amino) %>%
  as.data.frame() %>%
  dplyr::rename(log2FoldChange = FC_pmetab_7vs6_amino, pvalue = p_pmetab_7vs6_amino)

res_amino_7vs6$log2FoldChange <- abs_log(res_amino_7vs6$log2FoldChange)

output_p_amino <- EnhancedVolcano(res_amino_7vs6, pCutoff = 0.05,
                                  lab = rownames(res_amino_7vs6),
                                  x = 'log2FoldChange',
                                  y = 'pvalue')

# Insufficient differentially abundant plasma biomarkers
