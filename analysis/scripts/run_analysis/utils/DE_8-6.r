# Differential expression analysis for Cluster 8 vs 6
...
deg_8vs6 <- split(df, df$diffexpressed)
...
# Faecal untargeted metabolomics 
FC_fecal_8vs6 <- foldChngCalc(fmetab, 16, 13) 
p_fecal_8v6 <- NULL
for (i in 1:dim(fmetab)[1]){
  p_fecal_8v6[i]<-p.valcalc(as.numeric(fmetab[i,1:16]),as.numeric(fmetab[i,17:29]))
}

p_fecal_8v6 <- p.adjust(p_fecal_8v6, "BH", n = length(p_fecal_8v6) + dim(scfa)[1] + dim(bile)[1])

res_fmetab_8vs6 <- cbind(FC_fecal_8vs6, p_fecal_8v6) %>%
  as.data.frame() %>%
  dplyr::rename(log2FoldChange = FC_fecal_8vs6, pvalue = p_fecal_8v6)

res_fmetab_8vs6$log2FoldChange <- abs_log(res_fmetab_8vs6$log2FoldChange)
output_f_untargeted <- EnhancedVolcano(res_fmetab_8vs6, pCutoff = 0.05,
                                       lab = rownames(res_fmetab_8vs6),
                                       x = 'log2FoldChange',
                                       y = 'pvalue')
# Bile acid 
FC_bile_8vs6 <- foldChngCalc(bile, 16, 14) 

p_bile_8vs6 <- NULL
for (i in 1:dim(bile)[1]){
  p_bile_8vs6[i]<-p.valcalc(as.numeric(bile[i,1:16]),as.numeric(bile[i,17:30]))
}

p_bile_8vs6 <- p.adjust(p_bile_8vs6, "BH", n = length(p_bile_8vs6) + dim(scfa)[1] + dim(bile)[1])

res_bile_8vs6 <- cbind(FC_bile_8vs6, p_bile_8vs6) %>%
  as.data.frame() %>%
  dplyr::rename(log2FoldChange = FC_bile_8vs6, pvalue = p_bile_8vs6)

res_bile_8vs6$log2FoldChange <- abs_log(res_bile_8vs6$log2FoldChange)

output_f_bile <- EnhancedVolcano(res_bile_8vs6, pCutoff = 0.05,
                                 lab = rownames(res_bile_8vs6),
                                 x = 'log2FoldChange',
                                 y = 'pvalue')

# SCFA
FC_scfa_8vs6 <- foldChngCalc(scfa, 21, 14) 

p_scfa_8vs6 <- NULL
for (i in 1:dim(scfa)[1]){
  p_scfa_8vs6[i]<-p.valcalc(as.numeric(scfa[i,1:21]),as.numeric(scfa[i,22:35]))
}

p_scfa_8vs6 <- p.adjust(p_scfa_8vs6, "BH", n = length(p_bile_8vs6) + dim(scfa)[1] + dim(bile)[1])

res_scfa_8vs6 <- cbind(FC_scfa_8vs6, p_scfa_8vs6) %>%
  as.data.frame() %>%
  dplyr::rename(log2FoldChange = FC_scfa_8vs6, pvalue = p_scfa_8vs6)

res_scfa_8vs6$log2FoldChange <- abs_log(res_scfa_8vs6$log2FoldChange)

output_f_scfa <- EnhancedVolcano(res_scfa_8vs6, pCutoff = 0.05,
                                 lab = rownames(res_scfa_8vs6),
                                 x = 'log2FoldChange',
                                 y = 'pvalue')
...
# Untargeted
FC_pmetab_8vs6 <- foldChngCalc(pmetab_lab, 15, 8) 

p_pmetab_8vs6 <- NULL
for (i in 1:dim(pmetab_lab)[1]){
  p_pmetab_8vs6[i]<-p.valcalc(as.numeric(pmetab_lab[i,1:15]),as.numeric(pmetab_lab[i,16:23]))
}

p_pmetab_8vs6 <- p.adjust(p_pmetab_8vs6, "BH", n = length(p_pmetab_8vs6) + dim(amino_lab)[1])
res_pmetab_8vs6 <- cbind(FC_pmetab_8vs6, p_pmetab_8vs6) %>%
  as.data.frame() %>%
  dplyr::rename(log2FoldChange = FC_pmetab_8vs6, pvalue = p_pmetab_8vs6)

res_pmetab_8vs6$log2FoldChange <- abs_log(res_pmetab_8vs6$log2FoldChange)

output_p_untargeted <- EnhancedVolcano(res_pmetab_8vs6, pCutoff = 0.05,
                                       lab = rownames(res_pmetab_8vs6),
                                       x = 'log2FoldChange',
                                       y = 'pvalue')
# Amino
FC_pmetab_8vs6_amino <- foldChngCalc(amino_lab, 11, 9) 

p_pmetab_8vs6_amino <- NULL
for (i in 1:dim(amino_lab)[1]){
  p_pmetab_8vs6_amino[i]<-p.valcalc(as.numeric(amino_lab[i,1:11]),as.numeric(amino_lab[i,12:20]))
}

p_pmetab_8vs6_amino <- p.adjust(p_pmetab_8vs6_amino, "BH", n = length(p_pmetab_8vs6_amino) + length(res_pmetab_8vs6$pvalue))

res_amino_8vs6 <- cbind(FC_pmetab_8vs6_amino, p_pmetab_8vs6_amino) %>%
  as.data.frame() %>%
  dplyr::rename(log2FoldChange = FC_pmetab_8vs6_amino, pvalue = p_pmetab_8vs6_amino)

res_amino_8vs6$log2FoldChange <- abs_log(res_amino_8vs6$log2FoldChange)

output_p_amino <- EnhancedVolcano(res_amino_8vs6, pCutoff = 0.05,
                                  lab = rownames(res_amino_8vs6),
                                  x = 'log2FoldChange',
                                  y = 'pvalue')
