...
subprocess.call("Rscript scripts/preprocessing/utils/preprocess_biological_data.r", stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
...
file_path = "./data/results/alpha-diversity"
if os.path.isdir(file_path):
    for dirpath, dirnames, files in os.walk(file_path):
        if files:
            print('rarefaction already performed')
        else:
            subprocess.call("Rscript scripts/preprocessing/utils/metagenomics_alpha_diversity.r", stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
else:
    os.mkdir(file_path)
    subprocess.call("Rscript scripts/preprocessing/utils/metagenomics_alpha_diversity.r", stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)

# Extract diversity metrics and F-B ratio from metagenomic data
subprocess.call("Rscript scripts/preprocessing/utils/Fig_4_metagenomics_additional_analysis.r", stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
