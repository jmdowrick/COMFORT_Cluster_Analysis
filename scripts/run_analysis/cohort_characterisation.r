...
df_attributes <- readxl::read_excel(paste0(config$data_raw, config$file_PRO)) %>%
  dplyr::select(c(Diagnosis_CaseControls, Gender_numerical, AGE, CASE_CONTROL))

# Age ----  
age_characteristics <- group_by(df_attributes, Diagnosis_CaseControls) %>%
  dplyr::select(c(Diagnosis_CaseControls, AGE)) %>%
  drop_na() %>%
  summarise(avg = mean(AGE), std = sd(AGE))

age_differences <- dplyr::select(df_attributes, c(Diagnosis_CaseControls, AGE))
...
gender_characteristics <- group_by(df_attributes, Diagnosis_CaseControls) %>%
  dplyr::select(c(Diagnosis_CaseControls, Gender_numerical)) %>%
  table() %>%
  prop.table(margin = 1)
